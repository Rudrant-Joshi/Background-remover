...
import { Download, AlertCircle, RefreshCw, FileSpreadsheet } from 'lucide-react'
            ...
            <table className="w-full text-left border-collapse text-sm">
              <thead>
                <tr className="border-b border-card-border text-text-muted font-semibold">
                  <th className="py-3">Original</th>
                  <th className="py-3">Cutout</th>
              ...
              <tbody>
                {uploads.map((upload) => (
                  <tr key={upload.id} className="table-row">
                    <td className="py-3">
                      <div className="h-12 w-12 rounded overflow-hidden border border-card-border bg-background-secondary flex items-center justify-center">
                        {upload.original_url ? (
                          <img src={upload.original_url} className="h-full w-full object-cover animate-fade-in" alt="original" />
                        ) : (
                          <span className="text-[10px] text-text-muted font-bold">N/A</span>
                        )}
                      </div>
                    </td>
                    <td className="py-3">
                      <div 
                        className="h-12 w-12 rounded overflow-hidden border border-card-border flex items-center justify-center"
                        style={{ backgroundImage: "radial-gradient(circle, #222222 10%, transparent 11%)", backgroundSize: "6px 6px", backgroundColor: "#0A0A0A" }}
                      >
                        {upload.status === 'completed' ? (
                          <img src={upload.result_url} className="h-full w-full object-cover animate-fade-in" alt="cutout" />
                        ) : (
                          <span className="text-[10px] text-text-muted font-bold">FAIL</span>
                        )}
                      </div>
                    </td>
